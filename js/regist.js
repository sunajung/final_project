...
registForm.addEventListener('submit', (event) => {
    const idValue = registForm.id.value;
    const idCheckRes = idCheck(idValue);
    switch (idCheckRes) {
        case 1:
            alert('아이디를 입력해주세요.');
    ...
    const nameValue = registForm.name.value;
    if (!nameValue) {
        alert('이름을 입력해주세요.');
        event.preventDefault();
        return false;
    ...
    const useAgreeValue = registForm.useAgree.checked;
    if (!useAgreeValue) {
        alert('이용약관에 동의해주세요.');
        event.preventDefault();
        return false;
    ...
    const personalAgreeValue = registForm.personalAgree.checked;
    if (!personalAgreeValue) {
...
function idCheck(idValue) {
    ...
    for (var i = 0; i < idValue.length; i++) {
        if (!(isNum(idValue.charAt(i)) || isAlphabet(idValue.charAt(i)))) {
            alert('아이디는 영문과 숫자로만 이루어져야 합니다.');
            event.preventDefault();
            return 3;
        }
...
const duplicateURL = "./regist/duplicateCheck";
const duplicateCheckButton = document.getElementsByClassName('duplicate-check')[0];
...
registForm.id.addEventListener('propertychange change keyup paste input', e => {
    isPossibleID = false;
})
...
function disagreeFunc(checkBox) {
    alert('동의하지 않으시면 회원가입이 불가능합니다.');
